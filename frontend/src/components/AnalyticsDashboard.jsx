...
import { BarChart3, TrendingUp, RefreshCw, Layers, Server, Activity } from 'lucide-react';

export default function AnalyticsDashboard() {
  const [isRefreshing, setIsRefreshing] = useState(false);
  const [svgCurve, setSvgCurve] = useState('');
  const [backendStatus, setBackendStatus] = useState({ status: 'connecting', uptime: 0 });
  ...
  const fetchLiveGraph = async () => {
    ...
    try {
      const res = await fetch('/api/v1/graphs/risk-curve?condition=Viral+Respiratory&peak_day=4');
      if (res.ok) {
      ...
      if (healthRes.ok) {
        ...
        setBackendStatus({ status: 'operational', uptime: healthData.uptime_seconds });
      }
    } catch (e) {
      console.warn('Backend graph API not currently reachable, using local SVG preview:', e);
    } finally {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-3">
          <div className="flex items-center space-x-2 text-xs px-3 py-1.5 rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-mono">
            ...
            <span>FastAPI: {backendStatus.status === 'operational' ? 'Live & Connected' : 'Ready'}</span>
          </div>
          <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Dynamic Matplotlib SVG Graph Container */}
        <div className="p-6 bg-slate-900/90 border border-slate-800 rounded-2xl shadow-xl flex flex-col justify-between space-y-4">
          <div>
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div>
                ...
                <p className="text-xs text-slate-400">Rendered via backend Python Matplotlib service</p>
              </div>
          ...
          {/* SVG Container: renders live backend SVG if available, or vector fallback */}
          <div className="relative min-h-[190px] w-full bg-slate-950/80 rounded-xl border border-slate-800 p-2 flex items-center justify-center overflow-hidden">
            {svgCurve ? (
              <div
                className="w-full h-full flex items-center justify-center [&>svg]:w-full [&>svg]:h-auto [&>svg]:max-h-[180px]"
                dangerouslySetInnerHTML={{ __html: svgCurve }}
              />
            ) : (
              <svg viewBox="0 0 450 140" className="w-full h-full fill-none">
                ...
                <path d="M 40 105 Q 120 95, 180 50 T 290 35 T 370 70 T 430 45" stroke="#38bdf8" strokeWidth="3" strokeLinecap="round" />
                <path d="M 40 115 Q 140 110, 240 100 T 360 85 T 430 80" stroke="#10b981" strokeWidth="2" strokeDasharray="5 4" strokeLinecap="round" />
          ...
          <div className="p-3 rounded-lg bg-slate-800/50 border border-slate-700/50 text-xs text-slate-300 flex items-center justify-between">
            <span>Model Engine: <strong>FastAPI + Scikit-Learn</strong></span>
            <span className="font-mono text-slate-400">Stream: /api/v1/graphs/risk-curve</span>
          </div>
        </div>
      </div>
    </div>
