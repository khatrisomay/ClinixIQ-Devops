...
export default function TriageAcuityRibbon({ triageData, onUnlockReport, onTriggerER }) {
  ...
  return (
    <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-2">
      <div className="w-full rounded-2xl bg-white shadow-sm border border-[#dae2fd]/70 p-4 sm:p-5 flex flex-col xl:flex-row items-start xl:items-center justify-between gap-4 relative overflow-hidden">
        {/* Ambient Visual Indicator Accent */}
        <div className={`absolute left-0 top-0 bottom-0 w-2.5 rounded-l-2xl ${
        ...
        <div className="flex items-start gap-4 pl-2 sm:pl-3">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2.5 flex-wrap">
              ...
              <span className="text-xs text-[#3f4850] flex items-center gap-1 font-medium font-mono">
                <span className="material-symbols-outlined text-[14px]">schedule</span>
                Recommended Action: {isHighRisk ? 'Immediate / EMS Dispatch' : 'Within 12–24 Hours'}
              </span>
            ...
            <h1 className="text-lg sm:text-xl font-bold text-[#131b2e] mt-1 tracking-tight">
              {triageData?.condition 
                ? `Clinical Guidance: ${triageData.condition}`
                : 'Schedule Telehealth or Outpatient Urgent Care Evaluation'}
            </h1>

            <p className="text-xs sm:text-sm text-[#3f4850] max-w-3xl mt-0.5 leading-relaxed">
              {triageData?.action 
                ? triageData.action 
        ...
        {/* Quick Triage Actions & ER Escalation */}
        <div className="flex items-center gap-3 self-end xl:self-center shrink-0 w-full xl:w-auto justify-end pt-2 xl:pt-0">
          <button
            ...
            type="button"
            className="px-4 py-2 rounded-xl bg-[#e2e7ff] hover:bg-[#dae2fd] text-[#006194] text-xs font-semibold transition-all shadow-sm flex items-center gap-2 active:scale-95"
          >
          ...
          <button
            onClick={onTriggerER}
            type="button"
            className="px-4 py-2 rounded-xl bg-[#ba1a1a] hover:bg-[#ba1a1a]/90 text-white text-xs font-bold transition-all shadow-md shadow-rose-600/20 flex items-center gap-2 active:scale-95"
          >
