...
def test_mild_triage_prediction():
    req = TriageRequest(
        symptoms="mild sore throat and occasional cough with low energy", temperature=99.1
    )
    result = predict_triage(req)
    assert any(
        term in result.condition for term in ["Viral", "Respiratory", "Bronchitis", "Allergic"]
    )
