...
def generate_risk_curve_svg(condition_name: str = "Viral URI", peak_day: int = 4) -> str:
    ...
    # Plot lines
    ax.plot(days, patient_curve, color=PATIENT_COLOR, linewidth=2.8, label="Patient Symptom Severity", zorder=3)
    ax.plot(days, demographic_baseline, color=BASELINE_COLOR, linewidth=1.8, linestyle="--", label="Demographic Normal Baseline", zorder=2)
    ...
    ax.scatter([days[peak_idx]], [peak_val], color=ALERT_COLOR, s=70, zorder=5, label="Peak Acuity Index")
    ...
    ax.set_title(f"Clinical Risk Trajectory & Population Variance • {condition_name}", color=TITLE_COLOR, fontsize=11, fontweight="bold", pad=12)
    ...
    # Legend
    legend = ax.legend(loc="upper right", frameon=True, facecolor="#1e293b", edgecolor=AXIS_COLOR, fontsize=8)
    ...
    # Save to SVG buffer
    buffer = io.StringIO()
    fig.savefig(buffer, format="svg", bbox_inches="tight", facecolor=fig.get_facecolor(), edgecolor="none")
    plt.close(fig)
...
def generate_differential_bar_svg(differentials_data=None) -> str:
    ...
    if not differentials_data:
        conditions = ["Viral Bronchitis", "Influenza Type A", "Allergic Rhinitis", "Bacterial Pneumonia"]
        probs = [88, 64, 31, 14]
    else:
    ...
    colors = [ALERT_COLOR if p > 75 else (PATIENT_COLOR if p > 40 else BASELINE_COLOR) for p in probs]
    ...
    for bar in bars:
        ...
        ax.text(width + 2, bar.get_y() + bar.get_height() / 2, f"{int(width)}%",
                va="center", ha="left", color=TITLE_COLOR, fontsize=8, fontweight="bold")

    ax.set_xlim(0, 105)
    ax.set_xlabel("Probability Match (%)", color=TEXT_COLOR, fontsize=9)
    ax.set_title("Differential Diagnosis Probability Spectrum", color=TITLE_COLOR, fontsize=10, fontweight="bold")
